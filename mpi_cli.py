...
def mpi_simulate(jargs):
    ...
    if rank == 0:
        print(rank, "я пытаюсь отправлять")
...
class ReggaeSlave:
    ...
    def __init__(self, registry=TCPRegistryClient, service=MyService, mpi_state=None):
        self.state = self.STATE_INITIAL
...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        print(rank, "я тоже родился")
        reggy_slave = ReggaeSlave()
        reggy_slave.services_loop()
    else:
        print(rank, "не озадачен")
